...
CHECK_INTERVAL = int(os.getenv("CHECK_INTERVAL",
                               1800))  # Interval between each cycle in seconds
...
def main():
    ...
    while True:
        try:
            ...
            if not new_tweets:
                print(
                    f"No new tweets found for @{username}. Moving to the next user."
                )
            else:
                for tweet in new_tweets:
                    if str(tweet.id) not in reposted.get('users',
                                                         {}).get(username, []):
                        try:
                            ...
                            print(
                                f"Reposted tweet {tweet.id} from @{username} to {POST_TO}"
                            )

                            # Track reposted tweets
                            reposted.setdefault('users', {}).setdefault(
                                username, []).append(str(tweet.id))
            ...
            print(
                f"Waiting {CHECK_INTERVAL // 60} minutes before the next repost..."
            )
